...
const ColorInput: React.FC<ColorInputProps> = ({ label, value, onChange }) => {
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const normalized = normalizeHex(newText);
    if (normalized) {
      onChange(normalized);
    }
  };
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [copyStatus, setCopyStatus] = useState<'idle' | 'editor' | 'image'>(
    'idle'
  );
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-8 custom-scrollbar">
        ...
        <section className="space-y-3 bg-[#161616] -mx-5 px-5 py-4 border-b border-[#222]">
          ...
          <p className="text-[10px] text-gray-600 leading-tight">
            Use "Image Link" to embed a live-updating view of your activity grid
            in supported apps (Notion, etc.) or as an iframe.
          </p>
        </section>
        ...
        <section className="space-y-3">
          ...
          {config.granularity !== 'day' && (
            <div className="space-y-1 pt-2">
              ...
              <input
                ...
                onChange={e =>
                  updateConfig('itemsPerRow', parseInt(e.target.value))
                }
          ...
          {/* Dot Shape Selector */}
          <div className="space-y-1 pt-2">
            <label className="text-xs text-gray-400 block pb-1">
              Dot Shape
            </label>
            <div className="grid grid-cols-3 gap-1 bg-[#1a1a1a] p-1 rounded border border-[#333]">
              {['square', 'rounded', 'circle'].map(s => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (s === 'circle')
                      updateConfig('radius', config.dotSize / 2);
                    if (s === 'rounded' && config.radius === 0)
                      updateConfig('radius', 2);
                  }}
          ...
          <div className="space-y-1 pt-2">
            ...
            <input
              ...
              onChange={e =>
                updateConfig('maxIntensityThreshold', parseInt(e.target.value))
              }
              className="focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#111] rounded"
            />
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Background', id: 'bg' },
              { label: 'Text', id: 'text' },
              { label: 'Empty Day', id: 'empty' },
              { label: 'Filled Day', id: 'fill' },
            ].map(({ label, id }) => (
              <ColorInput
                key={id}
                label={label}
                value={config.colors[id as keyof AppConfig['colors']]}
                onChange={val =>
                  updateColor(id as keyof AppConfig['colors'], val)
                }
              />
            ))}
            <div className="col-span-2 pt-2 border-t border-[#222]">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={e =>
                    updateConfig('transparentBg', e.target.checked)
                  }
                  className="accent-orange-500 w-4 h-4 cursor-pointer focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#111] rounded-sm"
                />
                <label
                  htmlFor="check-transparent"
                  className="text-xs text-gray-400 cursor-pointer"
                >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <input
                ...
                onChange={e =>
                  updateConfig('showActiveLabel', e.target.checked)
                }
                className="accent-orange-500 w-4 h-4 cursor-pointer focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#111] rounded-sm"
              />
            </div>
              ...
              <div className="pl-2 border-l border-[#333]">
                ...
                <select
                  ...
                  onChange={e =>
                    updateConfig(
                      'activeLabelFormat',
                      e.target.value as ActiveLabelFormat
                    )
                  }
