...
export default function YearGridApp() {
  ...
  const selectedActivities = useMemo(() => {
    ...
    return activities.filter(a => 
      a.timestamp && new Date(a.timestamp).toISOString().split('T')[0] === selectedDate
    );
  ...
  const stats = useMemo(() => {
    ...
    const activeRecent = Object.keys(activityMap).filter(date => new Date(date) >= thirtyDaysAgo).length;
    ...
    <div className="flex h-[calc(100vh-140px)] bg-[#0a0a0a] text-white overflow-hidden relative rounded-2xl border border-[var(--app-panel-border)] shadow-2xl">
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col">
        ...
        <div className="grid grid-cols-4 gap-4 p-6 bg-white/5 border-b border-white/5 backdrop-blur-md">
            {[
                { label: 'Total Flow Actions', value: stats.total, sub: 'Lifetime logs', color: 'text-orange-500' },
                { label: 'Consistent Days', value: stats.activeRecent, sub: 'Last 30 days', color: 'text-emerald-500' },
                { label: 'Peak Momentum', value: stats.peak, sub: stats.peakDate ? new Date(stats.peakDate).toLocaleDateString() : '--', color: 'text-blue-500' },
                { label: 'Productivity Index', value: `${Math.round((stats.activeRecent / 30) * 100)}%`, sub: 'Consistency rate', color: 'text-violet-500' },
            ].map((s, i) => (
                <div key={i} className="space-y-1">
                    <p className="text-[10px] uppercase font-bold tracking-widest text-gray-500">{s.label}</p>
                    <div className="flex items-baseline gap-2">
                        <span className={`text-2xl font-black ${s.color}`}>{s.value}</span>
                        <span className="text-[10px] text-gray-600 font-medium truncate italic">{s.sub}</span>
                    </div>
                </div>
            ))}
        </div>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    <div className="space-y-1">
                        <p className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Flow Actions</p>
                    ...
                    {selectedActivities.length > 0 && (
                      <div className="space-y-2">
                        {selectedActivities.map((activity) => (
                          <div key={activity.id} className="p-2 rounded bg-white/5 border border-white/5 text-[11px] leading-tight transition-colors hover:bg-white/10">
                            <div className="flex justify-between gap-2 mb-1">
                                <span className="text-orange-400 font-bold uppercase tracking-tighter">{activity.action}</span>
                                <span className="text-gray-600 font-mono">{activity.timestamp ? new Date(activity.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</span>
                            </div>
                            <div className="text-gray-300 font-medium truncate">{activity.emailSubject}</div>
                            <div className="text-gray-500 truncate text-[10px]">{activity.emailSender}</div>
                          </div>
                        ))}
                      </div>
                    )}
