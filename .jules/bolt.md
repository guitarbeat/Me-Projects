...
## 2024-05-23 - [Date Parsing & Allocation in Loops]
**Learning:** `new Date()` object allocation in loops has overhead. Additionally, `toISOString().split('T')[0]` for local date formatting causes significant performance overhead and introduces a timezone shift bug.
**Action:** Replace `new Date()` allocations inside loops with string prefix matching where applicable, and replace `.toISOString().split('T')[0]` allocations with manual date formatting helpers.
