...
import { formatLocalDateToYYYYMMDD } from './dateUtils';
...
export const calculatePeriodInsights = (
  ...
  let streak = 0;
  const checkDate = allDates.includes(formatLocalDateToYYYYMMDD(today))
    ? today
    : yesterday;

  while (entries[formatLocalDateToYYYYMMDD(checkDate)]) {
    streak++;
