...
export const getDaysInMonth = (date: Date): (number | null)[] => {
  ...
  // ⚡ Bolt: Fast calculation for leap year and days in month without Date object allocation
  ...
  // ⚡ Bolt: Pre-allocate array size for performance instead of dynamic .push()
  const days = new Array<(number | null)>(totalLength);
