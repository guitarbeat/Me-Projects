    ...
    const { dataItems, startDayOffset, year } = useMemo(() => {
      try {
        ...
        if (granularity === 'day') {
          ...
          // Assign week indices
          let currentWeek = 0;
          const startDay = isMondayFirst
            ? (startDate.getDay() + 6) % 7
            : startDate.getDay();

          items.forEach((item, idx) => {
            const day = isMondayFirst
              ? (item.dayOfWeek! + 6) % 7
              : item.dayOfWeek!;
            if (idx > 0 && day === 0) {
              currentWeek++;
          ...
          <div style={gridStyle}>
            {dataItems.map((item, i) => (
              <DayCell
                ...
                selected={
                  selectedDate ===
                  (item.date ? formatDateLocal(item.date) : null)
                }
                label={item.label}
                item={item}
                index={i}
