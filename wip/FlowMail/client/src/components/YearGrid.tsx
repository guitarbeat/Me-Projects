...
const YearGrid: React.FC<YearGridProps> = React.memo(
  ({ config, activityMap = EMPTY_MAP, selectedDate, onSelectDate, className, domRef }) => {
    const {
      date,
      mode,
      granularity,
      ...
      try {
        ...
        if (granularity === 'day') {
          ...
          const getDayLabel = (d: Date, c: number) => {
            // Optimization: Avoid toLocaleDateString in loop, it is ~75x slower than manual formatting.
